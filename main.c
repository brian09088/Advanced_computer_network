...
int main(int argc,char *argv[]){
	...
	// Time Exceeded 表示封包在路由器上過期被丟棄。
	// decides to not send the ICMP time exceeded (i.e. TTL reached en-route)
        if(code == TIME_EXCEED){ 
            char hostname[1024] = "";
	...
	// 目標主機無法到達或無法回應
	// destination unreachable message (UDP-packet reached final host but port closed, proper behaviour though)
